...
[![Version](https://img.shields.io/badge/version-1.26.3-blue.svg)](./version.txt)
...
小規模な変更（バグ修正、軽微な改善等）向けの高速パス。Inception + Construction を統合した簡略フローで、設計ステップを省略して直接実装に進みます。`depth_level = "minimal"` かつ Unit数がちょうど1の場合に自動適用されます。
