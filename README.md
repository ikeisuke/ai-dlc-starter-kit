...
│       └── v1.0.0/            # v0.1.0を使って生成した実例（このプロジェクト自体の開発記録）
│           ├── prompts/       # v0.1.0で生成されたプロンプト
│           ├── templates/     # v0.1.0で生成されたテンプレート
│           ├── requirements/  # 要件定義成果物
│           ├── construction/  # 実装記録、progress.md
│           └── ...            # その他の成果物
│
└── prompts/
    └── setup-prompt.md        # セットアッププロンプト（v0.1.0、v1.0.0開発中）
```

**注**: 現在のバージョンは v0.1.0 です。`docs/versions/v1.0.0/` は v0.1.0 を使ってこのプロジェクト自体（v1.0.0）を開発した記録です。v1.0.0 を使ってセットアップすると `docs/aidlc/` に共通プロンプトが配置され、新しいディレクトリ構造になります。
