...
`gh_status` が `available` の場合、オープンIssueを確認する:
...
- `gh_status` が `available` 以外の場合: 「バックログ確認不可（GitHub CLI未利用可）」と表示する
...
`gh_status` が `available` 以外の場合: 「【警告】GitHub CLIが利用できません。バックログ確認ができません。」と表示しユーザーに続行可否を確認
