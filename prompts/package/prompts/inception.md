...
```bash
ls -d skills/aidlc/steps/inception/ 2>/dev/null
```
...
- **DEPLOYED_NOT_EXISTS**: 以下のお知らせを表示し、ステップ4へ進む
  ```text
  【お知らせ】skills/aidlc/steps/inception/ が見つかりません。
...
**空値の場合のフォールバック**: `check-backlog-mode.sh` は原則として常に有効値を返す。万一空値が返された場合は、`.aidlc/config.toml` の `[rules.backlog].mode` を直接読み取る。値が取得できない場合は `git` として扱う。

**判定結果表示**:
- `git` / `git-only`: ローカルファイル駆動（`.aidlc/cycles/backlog/`）
- `issue` / `issue-only`: GitHub Issue駆動（Issue作成、ラベル管理）

**mode=issue または issue-only で、`gh_status` が `available` 以外の場合**:
```text
...
**動作**: ステップ5と同じ手順（`.aidlc/cycles/rules.md` が存在すれば読み込む）を再実行し、切り替え先ブランチのルールを適用する。ファイルが存在しない場合はスキップ（通知なし）。
...
**AIレビュー**: Intent承認前に `skills/aidlc/steps/common/review-flow.md` に従ってAIレビューを実施すること。
...
**AIレビュー**: ユーザーストーリー承認前に `skills/aidlc/steps/common/review-flow.md` に従ってAIレビューを実施すること。
...
**AIレビュー**: Unit定義承認前に `skills/aidlc/steps/common/review-flow.md` に従ってAIレビューを実施すること。
...
squash を実行していない場合は、`skills/aidlc/steps/common/commit-flow.md` の「Inception Phase完了コミット」手順に従ってください。
...
`/aidlc construction` を実行し、Construction Phase を開始する。`automation_mode` の設定はそのまま引き継がれる。
...
ステップ14で確認した `backlog_mode` と `gh_status` を参照する。
...
`.aidlc/config.toml` の `[project].type` が `ios` の場合のみ実行。詳細手順は `docs/aidlc/guides/ios-version-update.md` を参照。
...
**【次のアクション】** `skills/aidlc/steps/common/commit-flow.md` の「Squash統合フロー」を読み込んで、Inception Phase完了squashの手順に従ってください。
...
squashを実行していない場合は、`skills/aidlc/steps/common/commit-flow.md` の「Inception Phase完了コミット」手順に従ってください。
...
**完了後、Construction Phaseに戻る場合**: `/aidlc construction` を実行
