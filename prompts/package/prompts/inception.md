...
**判定**:
- **STARTER_KIT_DEV**: 以下を表示し、ステップ5.5（サイクルモード確認）へ進む
  ```text
...
`false` の場合: 以下を表示し、ステップ5.5（サイクルモード確認）へ進む:
...
**判定**:
- **最新バージョン取得失敗**: ステップ5.5（サイクルモード確認）へ進む
- **CURRENT_VERSION が空**: ステップ5.5（サイクルモード確認）へ進む（aidlc.tomlなし）
- **LATEST_VERSION > CURRENT_VERSION**: アップグレード推奨を表示
  ...
  - **1 を選択**: セットアップを案内して終了
    ...
    ```
  - **2 を選択**: ステップ5.5（サイクルモード確認）へ進む
- **LATEST_VERSION = CURRENT_VERSION**: 以下を表示し、ステップ5.5（サイクルモード確認）へ進む
  ```text
...
- 既存名を選択した場合: 選択された名前を `cycle_name` としてコンテキスト変数に保持し、ステップ6へ進む
...
**スキップ条件**（以下のいずれかに該当する場合、このステップをスキップしてステップ6へ進む）:

- `cycle_name` が既に設定済みの場合（ステップ5.5で名前入力済みまたは既存名を選択済み）
- ステップ5.5で `mode = "ask"` から「1. 通常サイクル」を選択した場合
...
- `cycle_mode` を `"named"` に更新する（ステップ6以降の分岐との整合性を維持するため）
- ステップ6（サイクルバージョンの決定）へ進む（名前付きサイクルモードのバージョン提案フローが適用される）

**「1. いいえ」選択時**:

- ステップ6（サイクルバージョンの決定）へ進む（通常フロー）
...
- **main または master の場合**: ブランチ作成方式の設定を確認し、方式に応じて処理
  ...
  **7-4. main最新化チェック結果の表示**（setup-branch.sh実行する全経路 — branch/worktree/ask — に共通で適用）:
