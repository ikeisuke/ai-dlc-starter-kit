...
- **最新サイクル**: `ls -1 docs/cycles/ 2>/dev/null | grep -E '^v[0-9]+' | sort -V | tail -1` で取得し、`latest_cycle` として保持（バージョン確認に使用）
...
**動作**: ステップ5と同じ手順（`docs/cycles/rules.md` が存在すれば読み込む）を再実行し、切り替え先ブランチのルールを適用する。ファイルが存在しない場合はスキップ（通知なし）。
