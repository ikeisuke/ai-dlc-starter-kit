...
**残置ファイルの安全性**: Writeツールは同一パスへの書き込み時に既存ファイルを完全に上書きするため、前回セッションのファイルが残置していても内容の混入は発生しない。並行セッション環境では `aidlc-{purpose}-{timestamp}.txt` のように一意なサフィックスを付加すること。

#### 使用後の削除義務

一時ファイルは使用後に**必ず削除**する。削除はファイルを使用したコマンド実行直後に行う。セッション中断等で削除されなかった場合でも、次回書き込み時にWriteツールが上書きするため実害はない。`/tmp`のOS再起動時自動クリーンアップも補助的に機能する。

#### セキュリティ注意事項

- 一時ファイルに**機密情報（秘密鍵、トークン、パスワード等）を含めない**こと。一時ファイルの用途はコミットメッセージ、履歴コンテンツ、PRボディ等の非機密情報に限定する
