...
- 「将来的に対応したほうがよいでしょう。バックログファイルを作成しました: `.aidlc/cycles/backlog/chore-xxx.md`」

### バックログモード別フォーマットバリデーション【重要】

バックログ保存時、`.aidlc/config.toml` の `[rules.backlog].mode` に応じて許可される保存先が異なる。AIエージェントはバックログ保存前に必ずモードを確認し、禁止形式での保存を行わないこと。

**モード別 許可・禁止テーブル**:

| モード | ローカルファイル (`.aidlc/cycles/backlog/`) | GitHub Issue (`gh issue create`) |
|--------|:---:|:---:|
| `git` | 優先 | 許可 |
| `issue` | 許可 | 優先 |
| `git-only` | 専用 | **禁止** |
