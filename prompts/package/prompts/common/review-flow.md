...
1. レビュー種別に対応するスキルが利用可能か確認
   - `skill="reviewing-[type]"` で呼び出し可能かを確認
2. 利用可能なスキルが見つかったら、AIレビュー実行へ
3. すべて利用不可: AIレビュー不可として処理
...
3. **優先ツール決定・利用可否チェック**:
   ...
   - レビュー種別ごとに `skill="reviewing-[type]"` が利用可能か確認
   - 利用可能なスキルが見つかった → ステップ4へ
