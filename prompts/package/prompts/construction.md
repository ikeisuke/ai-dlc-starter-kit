...
`express_enabled=true` かつ `eligibility_result=eligible`（Inception Phase のステップ4bで判定済み）の場合、エクスプレスモードが適用される。

**検出条件**（`common/rules.md` の「エクスプレスモード仕様」セクションの適用条件に準拠）:
- `express_enabled=true` であること（Inception Phase のステップ14bで設定）
- 全 Unit 定義ファイルの「実装状態」セクション内「エクスプレス適格性」が `eligible` であること
...
**複数Unit時の動作**: エクスプレスモードで複数Unitが eligible の場合、Construction Phase の通常の Unit 選定ルール（ステップ9）がそのまま適用される。依存関係に基づく実行順序で最初の実行可能 Unit から開始し、Unit 完了後は次の実行可能 Unit に自動遷移する。全 Unit 完了後に Operations Phase へ遷移（またはコンテキストリセット）。

**注**: エクスプレスモードは「Inception→Construction のフェーズ間遷移」のコンテキストリセットをスキップするものであり、Construction Phase 内の Unit 間の遷移は既存の仕組みがそのまま適用される。
