...
各フェーズでAIレビューが実施されたか、履歴ファイルを読み取って自動確認する。

**注意**: AIレビュー対象タイミングは `docs/aidlc/prompts/common/review-flow.md` で定義されています（計画ファイル承認前、設計レビュー前、コード生成後の確認前、テスト完了後の確認前）。ここでは主要な2タイミングの実施有無を履歴から確認します。
...
if [ -f "$HISTORY_FILE" ]; then
    # ファイルが存在する場合、パターン検索
    if grep -q "AIレビュー完了\|レビュー反映" "$HISTORY_FILE"; then
        echo "IMPLEMENTED"
    else
...
- 判定基準: 「AIレビュー完了」または「レビュー反映」というキーワードが履歴に含まれているか
- **注意**: 「AIレビュー」単独では判定しない（「AIレビュースキップ」等の誤判定を防ぐため）
...
- 2を選択: 履歴に以下を記録して次のステップへ
  ...
  ```markdown
  ### AIレビュースキップ
  - **対象タイミング**: [設計レビュー（Phase 1 ステップ3） または 統合とレビュー（Phase 2 ステップ6）]
  - **スキップ理由**: [ユーザーが入力した理由]
  - **日時**: YYYY-MM-DD HH:MM:SS
  ```
