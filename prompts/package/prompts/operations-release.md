...
```bash
skills/aidlc/scripts/ios-build-check.sh "[選択されたパス]"
```
...
運用引き継ぎ（`.aidlc/cycles/operations.md`）の「バージョン確認設定」セクションを確認:
...
```bash
# 関連Issue番号を取得
skills/aidlc/scripts/pr-ops.sh get-related-issues {{CYCLE}}
...
```bash
skills/aidlc/scripts/pr-ops.sh find-draft
```

**ドラフトPRが見つかった場合**: ユーザーにReady化を確認し、承諾された場合:
```bash
skills/aidlc/scripts/pr-ops.sh ready {PR番号}
```
...
```bash
skills/aidlc/scripts/validate-git.sh uncommitted
```
...
```bash
skills/aidlc/scripts/validate-git.sh remote-sync
```
...
PRマージ前にAIレビューを実行し、品質を確認します。サブステップ0（ローカルレビュー）は常に実行します。サブステップ1以降はプロジェクト固有ルール（`.aidlc/cycles/rules.md`）にPRマージ前レビューのルールが定義されている場合に実行します。
...
3. **reviewingスキルによるレビュー**: `.aidlc/config.toml` の `[rules.reviewing]` 設定に基づき、適切なreviewingスキルを実行。レビュー種別は `.aidlc/cycles/rules.md` の「AIレビューツールの使用ルール」に従い選択する:
...
`.aidlc/cycles/rules.md` にPRマージ前レビューに関するプロジェクト固有ルールが定義されている場合、そのルールに従ってレビューを実行します。

**`rules.md` にルールが存在しない場合**: サブステップ1をスキップし、サブステップ2へ進みます。

指摘があれば修正を実施し、コミット・pushしてから次のサブステップへ進みます。

**注**: プロジェクト固有ルール（`.aidlc/cycles/rules.md`）にCodex PRレビューの絵文字リアクション検出が定義されている場合、サブステップ1で自動的に実行されます。
...
```bash
# 通常マージ（デフォルト）
skills/aidlc/scripts/pr-ops.sh merge {PR番号}

# Squashマージ
skills/aidlc/scripts/pr-ops.sh merge {PR番号} --squash

# Rebaseマージ
skills/aidlc/scripts/pr-ops.sh merge {PR番号} --rebase
```
