...
PRマージ前にAIレビューを実行し、品質を確認できます。プロジェクト固有ルール（`docs/cycles/rules.md`）にPRマージ前レビューのルールが定義されている場合、そのルールに従ってレビューを実行してください。ルールが定義されていない場合はこのステップをスキップし、7.13（PRマージ）へ進みます。

### サブステップ1: プロジェクト固有レビューの実行

`docs/cycles/rules.md` にPRマージ前レビューに関するプロジェクト固有ルールが定義されている場合、そのルールに従ってレビューを実行します。

**`rules.md` にルールが存在しない場合**: 7.12全体をスキップし、7.13（PRマージ）へ進みます。
