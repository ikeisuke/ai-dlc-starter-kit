...
PRマージ前にAIレビューを実行し、品質を確認します。サブステップ0（ローカルレビュー）は常に実行します。サブステップ1以降はプロジェクト固有ルール（`docs/cycles/rules.md`）にPRマージ前レビューのルールが定義されている場合に実行します。
...
3. **reviewingスキルによるレビュー**: `docs/aidlc.toml` の `[rules.reviewing]` 設定に基づき、適切なreviewingスキルを実行。レビュー種別は `docs/cycles/rules.md` の「AIレビューツールの使用ルール」に従い選択する:
