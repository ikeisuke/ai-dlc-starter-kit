...
## 7.6 Bash Substitution Check実行【CI対応】
...
**注意**: progress.mdでの「完了」は「PR準備完了」を意味します。この更新をGitコミット（7.8）に含めることで、PRに正確な状態が反映されます。7.9以降はPR準備完了後のレビュー・マージ作業です。
...
## 7.9 ドラフトPR Ready化【重要】

Inception Phaseで作成したドラフトPRをReady for Reviewに変更します（ステップ3で確認した `gh` ステータスを参照）。

**注意**: PR Ready化後は、バグ修正や追加要件がない限り**新たな変更**を加えないでください。progress.mdは既に7.7で「PR準備完了」として更新済みです。7.10でコミット漏れが見つかった場合は、漏れていたファイルのみ追加コミットしてください。
...
- **`status:ok`**: 次のステップ（7.11 リモート同期確認）へ進む
...
- **`status:ok`**: 次のステップ（7.12 mainブランチとの差分チェック）へ進む
...
- **behind**（`merge-base --is-ancestor` が失敗）:
  ...
  - 「2」選択時: 次のステップ（7.13 PRマージ前レビュー）へ進む

## 7.13 PRマージ前レビュー【推奨】

PRマージ前にローカルレビューとCodexレビューを実行し、品質を確認します。

### サブステップ1: ローカルレビュー

**Claude Code環境の場合**:

`/review` コマンドを実行し、PR差分のレビューを実施します。

指摘があれば修正を実施し、コミット・pushしてから次のサブステップへ進みます。

**その他の環境（Codex CLI / Gemini CLI等）**:

このサブステップをスキップし、サブステップ2へ進みます。
...
`DEFAULT_BRANCH` を決定します（優先順位順）:

1. 7.12で検出済みの `DEFAULT_BRANCH` 値
2. 未検出の場合: `gh pr view --json baseRefName --jq '.baseRefName'` でPRのベースブランチを取得
3. それも失敗時: `main` を使用
...
AIがレビュー結果を読み取り、指摘がある場合は修正を実施してコミット・pushします。

**利用不可の場合**:

```text
【情報】codex CLIが利用できません。Codexレビューをスキップします。
```
...
**実行条件**: サブステップ2（Codexレビュー）を実行した場合、かつ `gh:available` の場合のみ実行。いずれかの条件を満たさない場合はスキップして7.14（PRマージ）へ進みます。
...
- **`APPROVED`**: レビュー承認済み。7.14（PRマージ）へ進む
- **`CHANGES_REQUESTED`**: 以下を表示し、修正フローへ:
  ```text
  【レビュー指摘】PRに修正リクエストがあります。
  修正を実施し、コミット・pushした後、サブステップ2（Codexレビュー）から再実行してください。
  ```
  修正 → コミット → push → サブステップ2に戻る
- **`REVIEW_REQUIRED`** / **空文字列** / **`null`**（レビュー未完了）: 以下を表示:
  ```text
  【情報】Codex PRレビューがまだ完了していません。
...
**マージ前の確認**:
- サイクルPRの「Closes」セクションに全対応Issueが記載されているか確認（7.9で実施済み）
