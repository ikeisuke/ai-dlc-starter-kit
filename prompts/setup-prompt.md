...
- **開発ルール**（以下を含む）:
  ...
  - コード品質基準
  - Git運用の原則と例
  - プロンプト履歴管理【重要】: **必ず既存の `prompts/history.md` を読み込んでから末尾に追記**（既存履歴を絶対に削除・上書きしない）。Edit tool を使用し `## 実行履歴` セクションの最後に新エントリを追加。日時は `date '+%Y-%m-%d %H:%M:%S'` で取得。記録項目: 日時、フェーズ名、実行内容、プロンプト、成果物、備考
...
- **最初に必ず実行すること**（6ステップ）:
  ...
  4. 全Unit進捗分析と依存関係の解析:
     - `ls construction/units/*_implementation_record.md` で実装記録ファイルを確認し、各ファイルに「**完了**」マークがあるか grep で確認（**intent.mdやユーザーストーリーは読まない**）
     - `ls story-artifacts/units/` で全Unit定義ファイルを確認
     - 各Unit定義ファイルから「## 依存関係」セクションの「依存する Unit」を読み込み、依存関係マップを作成
     - **重要**: 完全なファイル内容は読まず、`grep -A 2 "## 依存関係" story-artifacts/units/*.md` で必要な部分のみ抽出
  5. 対象Unit決定（依存関係に基づく自動判断）:
     - **進行中のUnitがある場合**: そのUnitを継続（優先）
     - **進行中のUnitがない場合**: 以下のロジックで実行可能なUnitを判断
       1. 未着手のUnitの中から、依存元が全て完了しているUnitを抽出（実行可能Unitリスト）
       2. 実行可能Unitが0個: 「全Unit完了」または「依存関係が循環している」と判断
       3. 実行可能Unitが1個: 自動的にそのUnitを選択
       4. 実行可能Unitが複数: ユーザーに以下を提示し選択してもらう
          - 各Unitの名前、実装優先度、見積もり
          - 「推奨: [優先度Highまたは見積もりが小さいUnit]」と提案
          - ユーザーの選択を待つ
...
- 初期テンプレートを作成
- 記録ルール：
  - **重要**: 各プロンプト実行時に**必ず既存のhistory.mdを読み込んでから末尾に追記**する（既存の履歴を絶対に削除・上書きしてはいけない）
  - 追記方法: Edit tool を使用し、`## 実行履歴` セクションの最後に新しいエントリを追加
  - 日時取得：`date '+%Y-%m-%d %H:%M:%S'` コマンドを必ず使用
  - 記録項目：日時、フェーズ名、実行内容、プロンプト、成果物、備考
  - フォーマット: 各エントリは `### YYYY-MM-DD HH:MM:SS - フェーズ名` で始め、`---` で区切る
