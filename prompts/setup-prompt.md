...
> **重要**: セットアップ完了後、`prompts/additional-rules.md` をプロジェクトに合わせてカスタマイズすることを忘れないでください。詳細はファイル末尾の「セットアップ完了後の重要なステップ」を参照してください。
...
- **開発ルール**（以下を含む）:
  ...
  - 質問と回答の記録【重要】: **独自の判断をせず、不明点はドキュメントに `[Question]` タグで記録し `[Answer]` タグを配置、ユーザーに回答を求める**。ユーザーとは対話形式でやりとりし、回答を得てから次に進む
...
- **完了時の必須作業【重要】**: Inception Phaseで作成したすべてのファイルをGitコミット（メッセージ例: "feat: Inception Phase完了 - Intent、ユーザーストーリー、Unit定義を作成"）
...
- **フロー**（1つのUnitのみ）:
  ...
  2. 論理設計【対話形式】: 同上
  3. コード生成: 設計に基づいて実装
  4. テスト生成: BDD/TDDに従ってテスト作成
  5. 統合とレビュー: ビルド、テスト実行、レビュー
  （各ステップはテンプレートを参照）
...
- **Unit完了時の必須作業【重要】**: 各Unitで作成・変更したすべてのファイルをGitコミット（メッセージ例: "feat: [Unit名]の実装完了 - ドメインモデル、論理設計、コード、テストを作成"）
...
- **フロー**:
  1. デプロイ準備【対話形式】: 不明点は `[Question]` / `[Answer]` タグで記録し、ユーザーと対話しながら準備
  2. CI/CD構築【対話形式】: 同上
  3. 監視・ロギング戦略【対話形式】: 同上
  4. 配布（該当する場合）【対話形式】: 同上
  5. リリース後の運用【対話形式】: 同上
...
## 不明点と質問（Construction Phase中に記録）

[Question] ドメインモデル設計に関する不明点や確認したい内容をここに記録
[Answer] ユーザーからの回答をここに記録
...
[Question] 論理設計に関する不明点や確認したい内容をここに記録
[Answer] ユーザーからの回答をここに記録
...
Inception Phase を開始する前に、**必ず `prompts/additional-rules.md` をプロジェクトに合わせてカスタマイズしてください**。
...
#### `prompts/common.md` の技術スタック（greenfield の場合）
...
#### テンプレートファイル（オプション）
必要に応じて `templates/` 配下のテンプレートをプロジェクトに合わせて調整してください。
