...
- **最初に必ず実行すること**:
  ...
  0.5. **バックログ確認【重要】**:
     ...
     - **バックログファイルの存在確認**:
       - コマンド: `ls {{CYCLES_ROOT}}/backlog.md`
       - 存在しない場合: スキップ（初回セットアップ時）
       - 存在する場合: 次のステップへ
     - **バックログ内容の確認**:
       - バックログファイルを読み込む: `cat {{CYCLES_ROOT}}/backlog.md`
...
- **AI-DLCサイクル完了【重要】**:
  ...
  2.5. **バックログ記録【重要】**: 次サイクル以降で対応すべきタスクをバックログに記録
     ...
     - **バックログファイルの存在確認**:
       - コマンド: `ls {{CYCLES_ROOT}}/backlog.md`
       - 存在しない場合: テンプレート（`{{AIDLC_ROOT}}/templates/backlog_template.md`）から新規作成
       - 存在する場合: 既存ファイルに追記
     - **タスクの追記**:
     ...
     - **任意: 完了タスクの移動**:
       ...
       - 「Yes」の場合:
         - 完了したタスクを `{{CYCLES_ROOT}}/backlog.md` から削除
         - 削除したタスクを `{{CYCLES_ROOT}}/backlog_completed.md` に追記（完了サイクルと完了日を追加）
       - 「No」の場合: スキップ
