...
`[rules.milestone].enabled`（boolean、既定 `false`）を新設し、各 Milestone 関連ステップの**冒頭にガード分岐**を追加する。Unit 005 / 006 / 007 で確立した実装本体（5 ケース判定 / 冪等補完原則 / 手動復旧 3 パターン分岐 / Keep a Changelog 順序 / `gh_status != available` 時の exit 1 契約）は触らず、ガードのみを上乗せする。

具体的修正対象:

1. **`skills/aidlc/config/defaults.toml`**（正本）: `[rules.milestone]\nenabled = false` を追加
...
5. **`skills/aidlc/steps/inception/index.md`** Milestone（v2.4.0以降）参照箇所に「`enabled=true` のみ動作」の補足追加
6. **`skills/aidlc/steps/operations/01-setup.md`** ステップ 11 冒頭にガード追加
7. **`skills/aidlc/steps/operations/04-completion.md`** ステップ 5.5 冒頭にガード追加（**`enabled=false` 時は `gh_status != available` 時 exit 1 契約も発動しない**）
8. **`skills/aidlc/steps/operations/index.md`** §2.8 補助契約に `enabled` 条件追加
9. **`docs/configuration.md`** に新規セクション `[rules.milestone]` 追加
...
12. **`.aidlc/config.toml`**: メタ開発リポジトリ自体に `[rules.milestone].enabled = true` を明示設定
...
- Operations ステップ 5.5: `enabled=false` 時は `gh_status != available` 時 exit 1 契約を**自然文レベルで無効化宣言**（「後続の `gh_status` 判定 / `exit 1` 契約は一切実行しない」と明記）
...
| `gh_status != available` 時 exit 1 契約 | **`enabled=false` 時のみ無効化** | Unit 006 所有契約だが、opt-out 時は強制 close を要求しないため |
| LINK_FAILED 集約判定 exit 1 契約 | **`enabled=false` 時のみ無効化** | 同上 |
...
L33 の `inception.05-completion` 行と L113 / L208 の Milestone 参照箇所に `enabled=true` のみ動作する旨の補足を追加（最小編集）:

- L33: `Milestone（v2.4.0以降）` → `Milestone（v2.4.0以降、`[rules.milestone].enabled=true` のみ動作）`
...
- 表内の Milestone 関連行（`available` 以外の例外行）に「**かつ `[rules.milestone].enabled=true` のとき**」の条件を追記
- 補助契約に「**`[rules.milestone].enabled=false` のときは setup ステップ11 自体がスキップされ本契約は発動しない**」を追記
...
各ファイルの Milestone 関連箇所の先頭に「v2.4.0 以降、`[rules.milestone].enabled=true` のとき動作（既定 off）」の前提を 1 行追記:
...
- `glossary.md` L26 Milestone エントリの説明に「`[rules.milestone].enabled=true` のとき動作（既定 off）」を追記
...
2. **read-config.sh 動作確認**:
   - メタ開発リポ（明示設定）: `scripts/read-config.sh rules.milestone.enabled` → `true`、終了コード 0
   - 配布側想定 1（一時的に `.aidlc/config.toml` から `[rules.milestone]` セクションを削除して検証）: `scripts/read-config.sh rules.milestone.enabled` → `false`（defaults.toml フォールバック）、終了コード 0
   - 配布側想定 2（一時的に `defaults.toml` 側の `[rules.milestone]` も削除し、project / defaults 両方からキーを除去）: `scripts/read-config.sh rules.milestone.enabled` → 何も出力しない、終了コード 1。ガード側で `|| echo "false"` フォールバックで `false` 扱い
3. **opt-out 時のスキップ動作検証【最重要】**: `enabled=false` 時に Milestone 関連処理が **一切実行されないこと** を以下で検証:
   - 一時的に `.aidlc/config.toml` の `enabled=true` を `false` に変更して保存
...
6. **メタ開発フローでの実行可能性確認**: 本 Unit のコミット直後に `.aidlc/config.toml` の `enabled=true` で Operations Phase 04-completion ステップ 5.5 が動作することを本サイクル後続の Operations Phase で確認
...
- [ ] `skills/aidlc/config/defaults.toml` に `[rules.milestone]\nenabled = false` を追加
...
- [ ] Inception index.md / Operations index.md に `enabled=true` 補足追加
- [ ] `docs/configuration.md` に `[rules.milestone]` セクション追加
- [ ] 4 guides に「既定 off + 明示設定で有効化」追記
- [ ] `CHANGELOG.md` `[2.4.0]` 節 `### Added` に Unit G の opt-out 設定追加を追記
- [ ] `.aidlc/config.toml`（メタ開発リポジトリ）に `[rules.milestone]\nenabled = true` 設定追加
- [ ] `read-config.sh rules.milestone.enabled` がメタ開発リポで `true` を返すことを確認
...
- **メタ開発リポの自己整合**: 本 Unit のコミットに `.aidlc/config.toml` の `enabled=true` 設定追加を含めることで、本 Unit 完了直後の Operations Phase 04-completion ステップ 5.5 で v2.4.0 Milestone を close する流れを維持
