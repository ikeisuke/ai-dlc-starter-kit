...
- `skills/aidlc-setup/steps/03-migrate.md`（§9 以降または新規節 §9.5 / §10）に、以下のフローを追加:
  1. **マージ確認ガード**: 「PR をマージしましたか？」をユーザーに **1 回のみ** 確認（はい/いいえ/判断保留 など）。同意時は (2) → (3) を連続提示
  ...
  3. **HEAD 同期案内**（同上、削除案内の後）: `git fetch origin --prune` 実行後、worktree / 通常ブランチ / detached HEAD の現在状態に応じて 3 ケース分岐で `origin/main` の最新コミットに HEAD を一致させる
  4. **未コミット差分ガード**: HEAD 同期前に未コミット差分を検出した場合は同期を中断し、stash / commit / 中止のいずれかをユーザーに案内
