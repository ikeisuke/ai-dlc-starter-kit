...
- [ ] **通常系（Inception Phase で draft PR 作成済み）**: §7.6 時点で `pr_number` を `gh pr view` 等から確定させ、§7.7 最終コミットに含める手順が明示されている。
- [ ] **エッジケース（§7.8 で初回 PR 作成する経路、既存 L44〜L46 契約）**: §7.6 では `pr_number` を空のままとし `release_gate_ready` / `completion_gate_ready` の 2 スロットのみを §7.7 最終コミットに含めること、および `pr_number` は §7.8 の既存追加コミット契約（`chore: [{{CYCLE}}] PR番号記録 - operations/progress.md`）で永続化することが明文化されている（DR-001 に整合）。
- [ ] 固定スロットが既に `true` / 値設定済みで §7.6 に入った場合の扱い（上書き不要か明示再確認か）が一文以上で記述されている。
- [ ] §7.6 と §7.7 の記述が矛盾した場合に検知できるよう、共通参照先（`phase-recovery-spec.md` §5.3.5 を主、§7 を補助）を §7 冒頭またはヘッダから辿れる（リンク／相互参照が 1 箇所以上存在）。
