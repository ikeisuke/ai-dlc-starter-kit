...
- **コマンド置換の取り扱い**: 既存 `gh-project-cli.sh` の bash スクリプト内部ローカル変数代入での `$(...)` 使用は既存スタイル踏襲として許容する。一方、CLAUDE.md / `.aidlc/rules.md` で禁じられる **AI Bash プロンプト経由および git commit -m に渡す文字列内のコマンド置換** は新規導入しない（v2.6.2 Construction Phase 設計レビュー R1 指摘 #1 により文言を明確化）
