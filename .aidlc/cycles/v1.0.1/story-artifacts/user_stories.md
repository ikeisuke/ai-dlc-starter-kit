...
## Epic 7: セットアップファイルの最適化

### ストーリー 8.1: セットアップファイルの圧縮と分割
**優先順位**: Critical（緊急）

As a AI-DLC Starter Kitの利用者およびメンテナー
I want to セットアップファイルが適切なサイズに圧縮・分割されている
So that Claude Codeで確実に読み込むことができ、メンテナンスも容易になる

**受け入れ基準**:
- [ ] `prompts/setup-prompt.md` が300行程度に圧縮されている
- [ ] フェーズ別にファイルが分割されている：
  - [ ] `prompts/setup/inception.md`（500行程度）
  - [ ] `prompts/setup/construction.md`（500行程度）
  - [ ] `prompts/setup/operations.md`（500行程度）
  - [ ] `prompts/setup/common.md`（200行程度）
- [ ] 分割後も既存のセットアップ手順が正常に動作する
- [ ] 各ファイルがClaude Codeで読み込み可能なサイズである
- [ ] フェーズごとのメンテナンスが独立して行える
...
**背景**:
- Construction Phase（Unit 2実行準備中）にて、setup-prompt.mdが1746行に達し、読み込みに支障が発生
- セットアップファイルは全フェーズの起点であり、緊急性が高い
...
### Phase 7: セットアップファイル最適化（優先度: Critical）
- ストーリー 8.1: セットアップファイルの圧縮と分割

---

## 備考
- 全15ストーリー（5ストーリー追加、1ストーリーを緊急追加）
- Critical: 1ストーリー（緊急対応）
- Must-have: 7ストーリー
- Should-have: 5ストーリー
- Could-have: 2ストーリー
