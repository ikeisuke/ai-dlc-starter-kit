...
## 境界
- read-config.shの新旧キーフォールバ��クロジック追加は本Unitでは行わない（Unit 002, 003で実施）
- bin/check-size.sh自体の実行ロジッ���変更は最小限（キー不在時のハンドリングのみ）
...
## 技術的考慮事項
- preflight.mdの手順4（設定値取得）と手順5（オプションチェ���ク）を大幅に簡素化
