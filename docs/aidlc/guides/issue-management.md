...
| `status:waiting-for-review` | レビュー待ち | `#1D76DB` |
...
gh label create "status:waiting-for-review" --color "1D76DB" --description "レビュー待ち"
```
...
[Unit開始] → status:in-progress ←→ [ブロック] → status:blocked
     ↓                                   ↓
[Unit完了] → status:waiting-for-review   [解除] → status:in-progress
     ↓
[PRマージ] → (クローズ、ラベルなし)
```
