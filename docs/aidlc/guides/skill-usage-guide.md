...
**関連ドキュメント**: [AGENTS.md](../../../skills/aidlc/AGENTS.md)（AI-DLC全体の概要）
...
スキルの完全な一覧（名前空間・呼び出し名・状態・MP掲載状況）は [ai-tools.md の正規表](../../../skills/aidlc/steps/common/ai-tools.md) を参照してください。ai-tools.md がスキル一覧の正（Source of Truth）です。
...
# aidlc名前空間（レビュー系）
ln -s "<PROJECT_ROOT>/skills/reviewing-code" ~/.codex/skills/reviewing-code
ln -s "<PROJECT_ROOT>/skills/reviewing-architecture" ~/.codex/skills/reviewing-architecture
ln -s "<PROJECT_ROOT>/skills/reviewing-security" ~/.codex/skills/reviewing-security
ln -s "<PROJECT_ROOT>/skills/reviewing-inception" ~/.codex/skills/reviewing-inception
# aidlc名前空間（ワークフロー系）
...
reviewing-code -> /path/to/project/skills/reviewing-code
reviewing-architecture -> /path/to/project/skills/reviewing-architecture
reviewing-security -> /path/to/project/skills/reviewing-security
reviewing-inception -> /path/to/project/skills/reviewing-inception
squash-unit -> /path/to/project/skills/squash-unit
```

#### セットアップフローとの関係

- AI-DLCセットアップ（`/aidlc setup`）は Claude Code 用の `.claude/skills/` を自動作成しますが、`~/.codex/skills` は対象外です
...
```text
「コードレビューして」と言われたら:
→ skills/reviewing-code/SKILL.md を読んで、その手順に従う

「アーキテクチャレビューして」と言われたら:
→ skills/reviewing-architecture/SKILL.md を読んで、その手順に従う

「セキュリティレビューして」と言われたら:
→ skills/reviewing-security/SKILL.md を読んで、その手順に従う

「Inceptionレビューして」と言われたら:
→ skills/reviewing-inception/SKILL.md を読んで、その手順に従う
```
...
```json
{
  ...
  "resources": [
    ...
    "file://skills/reviewing-architecture/SKILL.md",
    "file://skills/reviewing-security/SKILL.md",
    "file://skills/reviewing-inception/SKILL.md"
  ],
...
```text
.claude/skills/
├── reviewing-code/           → ../../skills/reviewing-code/           (シンボリックリンク)
├── reviewing-architecture/   → ../../skills/reviewing-architecture/   (シンボリックリンク)
├── reviewing-security/       → ../../skills/reviewing-security/       (シンボリックリンク)
├── reviewing-inception/      → ../../skills/reviewing-inception/      (シンボリックリンク)
├── aidlc-setup/              → ../../skills/aidlc-setup/              (シンボリックリンク)
├── squash-unit/              → ../../skills/squash-unit/              (シンボリックリンク)
...
| 名前 | 使用可否 | 理由 |
|------|----------|------|
| `reviewing-code`, `reviewing-architecture`, `reviewing-security`, `reviewing-inception`, `aidlc-setup`, `squash-unit` | 不可 | スターターキット予約名（シンボリックリンク） |
...
1. スキルファイルが存在するか確認
   ...
   ls skills/
   ```
2. セットアップが完了しているか確認（`/aidlc setup` を実行）
