...
- **設計レビュー**: フィードバックを反映する必要がある作業
  - ただし例外あり: `prompts/package/prompts/common/review-flow.md` ステップ5.5のセルフレビューフローでは「レビュー実行のみ」をサブエージェントに委譲可能
  - 委譲範囲: 対象ファイルの読み取り・評価・結果出力のみ（読み取り専用。ファイル編集・コマンド実行・外部通信は禁止）
  - メインエージェント責務: 修正判断・反復制御・履歴記録
