...
`docs/cycles/{{CYCLE}}/backlog.md` を確認し、Operations Phaseで対応できる項目があれば対応を検討
...
サイクル固有バックログ（`docs/cycles/{{CYCLE}}/backlog.md`）を整理し、共通バックログに反映する。

#### 3.1 サイクル固有バックログの読み込み
`docs/cycles/{{CYCLE}}/backlog.md` を読み込む

#### 3.2 「このサイクルで発見した項目」の処理

| 状態 | 移動先 |
|------|--------|
| 対応済み | `docs/cycles/backlog-completed.md` に移動（完了サイクル・完了日・対応内容を追記） |
| 未対応 | `docs/cycles/backlog.md`（共通）に移動 |

#### 3.3 「共通バックログから対応する項目」の処理

| 状態 | 処理 |
|------|------|
| 対応済み | `docs/cycles/backlog-completed.md` に移動、`docs/cycles/backlog.md`（共通）から削除 |
| 未対応 | 共通バックログに残す（サイクル固有から転記情報を削除） |

#### 3.4 新規項目の追記
次サイクルに引き継ぐ新たなタスクがある場合、`docs/cycles/backlog.md`（共通）に記録：
...
#### 3.5 最終更新の記録
`docs/cycles/backlog.md` と `docs/cycles/backlog-completed.md` の「最終更新」セクションを更新
