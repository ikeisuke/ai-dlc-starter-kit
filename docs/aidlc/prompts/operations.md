...
Ready化の前に、サイクルPRの「Closes」セクションを確認します。
...
2. Unit定義ファイルから関連Issue番号を取得:
   ```bash
   grep -h "対応Issue\|関連Issue" docs/cycles/{{CYCLE}}/story-artifacts/units/*.md | grep -oE '#[0-9]+'
   ```

3. PRの「Closes」セクションに全関連Issueが記載されているか確認
...
「Closes」セクションに不足しているIssue番号を `Closes #XX` 形式で追加してください。
