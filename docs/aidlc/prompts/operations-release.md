...
PRマージ前にAIレビューを実行し、品質を確認できます。プロジェクト固有ルール（`docs/cycles/rules.md`）にPRマージ前レビューのルールが定義されている場合、そのルールに従ってレビューを実行してください。ルールが定義されていない場合はこのステップをスキップし、7.14（PRマージ）へ進みます。

### サブステップ1: プロジェクト固有レビューの実行

`docs/cycles/rules.md` にPRマージ前レビューに関するプロジェクト固有ルールが定義されている場合、そのルールに従ってレビューを実行します。

**`rules.md` にルールが存在しない場合**: 7.13全体をスキップし、7.14（PRマージ）へ進みます。
...
**実行条件**: サブステップ1でPRレビュー（GitHub上のレビュー機能を使用するもの）を実行した場合、かつ `gh_status` が `available` の場合のみ実行。いずれかの条件を満たさない場合はスキップして7.14（PRマージ）へ進みます。
