...
- **behind**（`merge-base --is-ancestor` が失敗）:
  ...
  - 「2」選択時: 次のステップ（6.6.8 PRマージ前レビュー）へ進む
...
1. 6.6.7で検出済みの `DEFAULT_BRANCH` 値
...
**実行条件**: サブステップ2（Codexレビュー）を実行した場合、かつ `gh:available` の場合のみ実行。いずれかの条件を満たさない場合はスキップして6.7（PRマージ）へ進みます。
...
- **`APPROVED`**: レビュー承認済み。6.7（PRマージ）へ進む
