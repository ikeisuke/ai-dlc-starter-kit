...
各フェーズでAIレビューが実施されたか確認する。

**注意**: AIレビュー対象タイミングは `docs/aidlc/prompts/common/review-flow.md` で定義されています（計画ファイル承認前、設計レビュー前、コード生成後の確認前、テスト完了後の確認前）。ここでは主要な2タイミングを確認します。

**確認項目**:

- 設計レビュー（Phase 1 ステップ3）でAIレビューを実施したか
- 統合とレビュー（Phase 2 ステップ6）でAIレビューを実施したか
