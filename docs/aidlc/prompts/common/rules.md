...
```bash
docs/aidlc/bin/read-config.sh rules.upgrade_check.enabled --default "false"
```
...
### バックログモード別フォーマットバリデーション【重要】

バックログ保存時、`docs/aidlc.toml` の `[rules.backlog].mode` に応じて許可される保存先が異なる。AIエージェントはバックログ保存前に必ずモードを確認し、禁止形式での保存を行わないこと。

**モード別 許可・禁止テーブル**:

| モード | ローカルファイル (`docs/cycles/backlog/`) | GitHub Issue (`gh issue create`) |
|--------|:---:|:---:|
| `git` | 優先 | 許可 |
| `issue` | 許可 | 優先 |
| `git-only` | 専用 | **禁止** |
...
- **優先**: デフォルトの保存先として使用する
- **許可**: 状況に応じて使用可能
- **専用**: この保存先のみ使用する（排他モード）
- **禁止**: この保存先への記録は一切行わない

**フォールバック**: `backlog_mode` が未取得または不正な値の場合は `git` として扱う。

**バリデーション手順**:

1. バックログ保存前に `backlog_mode` コンテキスト変数（プリフライトチェックで取得済み）を確認する
2. 上記テーブルに照合し、禁止された保存先への保存を試みていないか検証する
3. 禁止形式への保存を検出した場合、許可された保存先が技術的に利用可能であれば正しい形式で保存し直す。利用不可の場合（例: `issue-only` + gh CLI不可用）はユーザーに手動登録を依頼する

**バリデーションエラー時の期待動作**:

```text
【バリデーションエラー】{mode}モードでは{禁止された保存先}は禁止されています。{許可された保存先}に保存します。
```

例:
- `【バリデーションエラー】issue-onlyモードではローカルファイル作成は禁止されています。GitHub Issueに保存します。`
- `【バリデーションエラー】git-onlyモードではGitHub Issue作成は禁止されています。ローカルファイルに保存します。`

**注意**: このバリデーションはAIエージェントがプロンプトルールに従って実行するプロンプトベースの制約であり、実行時チェック機構（スクリプト等）ではない。
