...
#### 質問と回答の記録【重要】
**独自の判断をせず、不明点はドキュメントに `[Question]` タグで記録し `[Answer]` タグを配置、ユーザーに回答を求める**。**一問一答形式で対話する**：1つの質問をして回答を待ち、回答を得てから次の質問をする。**複数の質問をまとめて提示してはいけない**。
...
#### プロンプト履歴管理【重要】
**必ず `prompts/history.md` のファイル末尾に追記**してください（既存履歴を絶対に削除・上書きしない）。

追記手順:
1. 日時を取得: `date '+%Y-%m-%d %H:%M:%S'`
2. Bash heredoc で末尾に追記:
   ```bash
   ...
   **備考**: [特記事項]
   ...
   EOF
   ```
3. **ファイルを読み込む必要はありません**（テンプレートが先頭にあるため、フォーマットを確認したい場合のみ読む）

**警告**: Write tool や Edit tool で上書き・編集してはいけません。必ず heredoc で追記してください。
