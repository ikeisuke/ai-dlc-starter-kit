...
|------|------|------------------|--------|
| ファイルヘッダー | なし | 新規作成時に`# ... 履歴`を追加 | 形式変更（初回のみ） |
| 見出し形式 | `## TIMESTAMP` | `## TIMESTAMP` | 完全互換 |
| フェーズ表示 | 同一 | 同一 | 完全互換 |
| Unit表示 | `[Unit名]` | `{N}-{slug}（{name}）` | 形式変更（情報量増加） |
| 成果物形式 | 1行プレーンテキスト | 箇条書きリスト（バッククォート付き） | 形式変更（可読性向上） |
| 成果物なしの場合 | `- **成果物**: [プレースホルダー]` | 成果物行を出力しない | 形式変更 |
| 区切り線 | `---` | `---` | 完全互換 |
| ファイル名（construction） | `construction_unit{N}.md` | `construction_unit{NN}.md`（2桁ゼロパディング） | 形式変更 |

**結論**: 基本構造は互換。以下の変更点を許容：
- 新規ファイル作成時のヘッダー追加（可読性向上）
- 成果物の箇条書き形式（可読性向上）
- 成果物なし時の行省略（冗長性削減）
- constructionファイル名のゼロパディング（ソート改善）
...
**現在**:
```markdown
`docs/cycles/{{CYCLE}}/history/inception.md` に履歴を追記（heredoc使用、日時は `date '+%Y-%m-%d %H:%M:%S %Z'` で取得）
```
