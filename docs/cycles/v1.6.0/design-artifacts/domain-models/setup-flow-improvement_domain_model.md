...
| branch | 現在のディレクトリでブランチ作成・切り替え | `git checkout -b` |
...
flowchart TD
    ...
    B -->|Yes| C[選択肢提示]
    C --> D{ユーザー選択}
    D -->|worktree| E{ブランチ存在確認}
    D -->|branch| F[git checkout -b]
    D -->|continue| G[警告表示]
    E -->|存在する| H[既存ブランチでworktree作成]
    E -->|存在しない| I[新規ブランチでworktree作成]
    H --> J{既存worktree確認}
    I --> J
    J -->|存在する| K[既存worktree使用確認]
    J -->|存在しない| L[worktree作成確認]
    K --> M[パス表示・終了]
    L --> N{作成実行}
    N -->|成功| O[移動案内表示]
    N -->|失敗| P[手動コマンド提示]
