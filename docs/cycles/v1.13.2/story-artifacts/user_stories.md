...
### ストーリー 8: progress.md更新タイミングの修正
**優先順位**: Must-have

As a AI-DLC利用者
I want to Operations Phase完了時にprogress.mdの「完了」更新がPRマージに含まれる
So that マージ後のmainブランチでも正確な進捗状態が記録される

**受け入れ基準**:
- [ ] ステップ6.5（Gitコミット）の前にprogress.mdでステップ6を「完了」に更新する指示が追加される
- [ ] マージ後のmainブランチで `docs/cycles/{cycle}/operations/progress.md` のステップ6が「完了」になっている
- [ ] 現在の「ステップ完了時: progress.mdでステップ6を「完了」に更新」の記述がPRマージ前の適切な位置に移動される

**技術的考慮事項**:
- 変更対象: `prompts/package/prompts/operations.md`
- ステップ6.5 Gitコミットの直前でprogress.md更新を行う
- 現在のフロー: 6.5コミット → 6.6 PR Ready化 → 6.7マージ → 完了更新（❌ マージに含まれない）
- 修正後フロー: 完了更新 → 6.5コミット → 6.6 PR Ready化 → 6.7マージ（✅ マージに含まれる）

---

## 受け入れ基準のチェック観点

| チェック項目 | US1 | US2 | US3 | US4 | US5 | US6 | US7 | US8 |
|-------------|-----|-----|-----|-----|-----|-----|-----|-----|
| 具体性 | OK | OK | OK | OK | OK | OK | OK | OK |
| 検証可能性 | OK | OK | OK | OK | OK | OK | OK | OK |
| 完全性 | OK | OK | OK | OK | OK | OK | OK | OK |
| 独立性 | OK | OK | OK | OK | OK | OK | OK | OK |
...
- US8はUS3（operations.md行数削減）と同じファイルを変更するが、US3完了後に実施することで競合を回避
- 各ストーリーは単独で完了可能
