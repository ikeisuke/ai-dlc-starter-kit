...
## 成功基準
- codex-review、claude-review、gemini-reviewが削除され、reviewing-code、reviewing-architecture、reviewing-securityの3スキルに再編されている
- 各レビュースキルが種別に応じた観点（チェックリスト・基準）を提供している
- 各レビュースキルから複数のAIツール（Codex/Claude/Gemini）を呼び出せる
- ghスキルが削除され、AI-DLCプロンプトでgh未使用時の分岐が適切に動作する
- jjスキルがagentskills.io仕様に準拠している（name/description/Progressive Disclosure）
- aidlc-upgradeスキルが #181 の効率化を含め、agentskills.io仕様に準拠している
- 全スキルの SKILL.md が agentskills.io の frontmatter 仕様を満たしている
...
- agentskills.io 仕様: SKILL.md body は500行以下を推奨
...
3. **gh スキル削除 + プロンプト調整**: gh未使用時のフロー改善
4. **jj スキル改善**: agentskills.io ベストプラクティス準拠
5. **aidlc-upgrade スキル改善**: #181 対応 + agentskills.io ベストプラクティス準拠
6. **関連ファイル更新**: review-flow.md、AGENTS.md、skill-usage-guide.md、シンボリックリンク等
