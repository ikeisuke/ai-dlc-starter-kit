...
└── references/
    └── session-management.md             # セッション管理詳細（Unit 001/002と同一、~153行）
```
...
## レビュー観点
### OWASP Top 10
[チェック項目5つ: インジェクション、アクセス制御不備、暗号化の失敗、XSS、SSRF]
### 認証・認可
[チェック項目5つ: 認証メカニズム、セッション管理、最小権限、権限昇格防止、トークン管理]
### 依存脆弱性
[チェック項目4つ: 既知脆弱性検出、バージョン管理、サプライチェーン、不要依存排除]
