...
- **属性**:
  ...
  - preferredTool: ToolName - ai_toolsから決定された優先ツール名
...
### ToolResolver（ツールリゾルバ）

- **責務**: ai_tools設定から優先ツールを決定し、スキルに伝達する
- **操作**: resolve(aiTools: ToolName[]): ToolName
  - ai_toolsリストの先頭から利用可能なツールを返す
  - 結果は「優先ツール: [name]」として引数テキストに含める
  - スキルが解釈できない場合のフォールバックはスキル側の責務
...
classDiagram
    ...
    ToolResolver --> ToolName : resolves
...
- **優先ツール（PreferredTool）**: ai_tools設定で指定されたツールの優先順位。最善努力のヒントとしてスキルに伝達される
