...
| 条件 | 対応 | 判定方法 |
|------|------|----------|
| dasel未インストール | **AIが直接読み込み** | `command -v dasel` |
| ファイル不在 | 新規セットアップフローへ（既存動作維持） | `[ -f docs/aidlc.toml ]` |
| キー欠損 | デフォルト値を使用 | 出力が空文字列 |
| TOML構文エラー | エラーメッセージ表示・処理中断 | exit code != 0 && stderr出力あり |

**フォールバック方式の変更点**:
- **旧方式**: grep/sed/awkによる正規表現パース（複雑で保守困難）
- **新方式**: AIがReadツールで `docs/aidlc.toml` を読み込み、TOML構造を解析
- **メリット**: 複雑なシェルスクリプト不要、TOMLの構造変更に強い

**プロンプト記述例**:
```bash
if command -v dasel >/dev/null 2>&1; then
    VALUE=$(dasel -f docs/aidlc.toml -r toml '.key.path' 2>/dev/null || echo "DEFAULT")
else
    VALUE=""  # AIが docs/aidlc.toml を読み込んで取得（下記参照）
fi
[ -z "$VALUE" ] && VALUE="DEFAULT"
```

**AI読み込み方式**:
- AIはReadツールで `docs/aidlc.toml` を**そのまま**読み込む
- コメント行・空行は**AIが解釈時に無視**（前処理でのgrep除外は行わない）
- 理由: `grep -v "^#"` は複数行文字列やインデントコメントで問題を起こす可能性があるため
...
- **設定読み込みパターン**: dasel優先 + AI読み込みフォールバック（前処理なし）
