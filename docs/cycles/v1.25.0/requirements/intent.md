...
## 開発の目的
AI-DLCの開発者体験（DX）を向上させる。小規模変更をより素早く開始できるエクスプレスモードのインスタント起動、フェーズ開始時の環境品質保証（プリフライトチェック・設定値一括取得）、PRレビュー自動化の改善（Codex絵文字リアクション検出）、およびスキル基盤の整理（.kiro/skills→.agent/skills移行）を実施する。
...
- .agent/skills への移行により、スキル基盤を将来の拡張に備えて整理
...
- .kiro/skills が .agent/skills に移行され、リポジトリ内の `.kiro/skills` 参照が意図した例外（過去サイクルの履歴ドキュメント等）を除き0件である（`rg ".kiro/skills"` で検証）。破壊的変更としてCHANGELOGに明記し、アップグレード手順を提供する
...
- .kiro/skills → .agent/skills の移行はディレクトリ移動と参照パス変更のみ（スキル定義形式の変更は含まない）。破壊的変更として扱い、CHANGELOGに明記しアップグレード手順を提供する
...
- #347 .kiro/skills廃止と.agent/skills作成
...
[Question] #347 の .agent/skills ディレクトリパスは確定か？
[Answer] はい、.agent/skills で確定。
