...
- **位置**: 「## 完了時の必須作業【重要】」セクション内、「### 1. 履歴記録」の前
- **責務**: サイクルラベルの存在確認と自動作成、関連Issueへのラベル付与
- **前提条件**:
  - BACKLOG_MODE = "issue"
  - GitHub CLIが利用可能かつ認証済み
...
- **パラメータ**:
  - label_name - ラベル名
  - description - 説明文
  - color - 6桁Hexカラーコード（#なし、例: FBCA04）
- **戻り値**: 成功時はメッセージ、失敗時はエラー
- **副作用**: GitHubリポジトリにラベルを作成

#### Issueへのラベル付与

- **コマンド**: `gh issue edit {issue_number} --add-label "{label_name}"`
- **パラメータ**:
...
1. BACKLOG_MODE変数が "issue" であることを確認
   - "issue" 以外の場合: このセクションをスキップ
2. GitHub CLI認証状態を確認（ステップ0.7で実施済み）
   - 未インストールまたは未認証の場合: 警告を表示してスキップ

**ステップ**:
1. ユーザーにラベル作成の確認を求める（デフォルト: 作成する）
   - 「いいえ」の場合: スキップして次のステップへ
2. 各ラベルについて存在確認（`gh label list` + jqフィルタ）
   - 注意: ラベル数が多いリポジトリでは `--limit` オプションを指定
3. 存在しないラベルのみ作成
4. 作成結果をサマリ表示（作成数、スキップ数）
...
2. 存在しない場合に作成（色: C5DEF5 薄青）
3. Unit定義ファイルの「関連Issue」セクションからIssue番号を抽出
4. 各Issueにサイクルラベルを付与（`gh issue edit {number} --add-label "cycle:{{CYCLE}}"`）
5. ラベル付与結果をサマリ表示
...
- カラーコードは `#` なしで指定（例: `FBCA04`、`#FBCA04` は不可）
- ラベル数が多いリポジトリでは `gh label list --limit 100` などで上限を指定
