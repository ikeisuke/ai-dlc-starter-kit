...
### プロンプト品質向上
- [ ] setup-prompt.md, inception.md, construction.md, operations.md から複合コマンド（`&&`, `||`, `|`, `VAR=$(cmd)`）が削除されている
- [ ] 許可リストに `gh label list:*`, `gh label create:*` が追加されている

### セットアップフロー最適化
- [ ] セットアップ時のサイクルディレクトリ作成で確認ダイアログが表示されない
- [ ] セットアップで決定したIntent/スコープがインセプションに引き継がれている
