...
↓ Operations Phase の rsync で反映

docs/aidlc/                         # ← 配布先（利用時の参照先）
├── bin/
│   └── read-config.sh              # 利用時はこのパスで呼び出し
└── ...
```

**注意**: 実装は `prompts/package/` で行い、`docs/aidlc/` は直接編集しない（メタ開発ルール）。
...
| `--default` | No | キーが存在しない場合のデフォルト値（文字列として渡す） |

**`--default` の制約**:
- 文字列値のみサポート（配列・テーブルはサポート外）
- クォート・エスケープは呼び出し側で処理

#### 戻り値

- 成功時（値あり）: 設定値を標準出力に出力、終了コード 0
- キー不在（デフォルトあり）: デフォルト値を出力、終了コード 0
- キー不在（デフォルトなし）: 何も出力しない、終了コード 1
- エラー（dasel未インストール等）: エラーメッセージを標準エラー出力、終了コード 2

**備考**: 終了コードで「値あり」「キー不在」「エラー」を区別可能。空文字列が設定されている場合は終了コード 0 で空文字列を出力。
...
- 空文字列と未定義の区別は終了コードで判断（0=値あり、1=キー不在、2=エラー）
