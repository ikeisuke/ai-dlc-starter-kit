...
### PRマージ後mainに移動してpullする手順の明確化
- **発見日**: 2025-12-09
- **発見サイクル**: v1.2.4セットアップ時
- **概要**: PRマージ後にmainブランチに移動してpullする手順をドキュメント化
- **詳細**: サイクル完了後のPRマージ後、次サイクル開始前にmainに移動してpullする手順が明確でない
- **優先度**: 中

### Operations Phaseのバージョン更新でアップグレードが正しく動作しない
- **発見日**: 2025-12-09
- **発見サイクル**: v1.2.4セットアップ時
- **概要**: Operations Phaseでversion.txtを更新しても、次サイクルでアップグレード対象として検出されない
- **詳細**: v1.2.3サイクル完了後、version.txtを1.2.3に更新したが、次サイクルセットアップ時に「現在のバージョン: 1.2.2」と表示された。aidlc.tomlのstarter_kit_versionが更新されていない可能性
- **対応案**:
  1. Operations Phaseでversion.txt更新時にaidlc.tomlのstarter_kit_versionも同時更新
  2. アップグレード検出ロジックの見直し
  3. バージョン更新手順の明確化
- **優先度**: 高
