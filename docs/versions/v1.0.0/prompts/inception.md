...
- 回答を得てから `{{VERSIONS_ROOT}}/{{VERSION}}/requirements/intent.md` を作成
- テンプレート: `{{AIDLC_ROOT}}/templates/intent_template.md` を参照

### 2. 既存コード分析（brownfield のみ）

- 既存のコードベースを分析し、技術スタック、アーキテクチャ、制約を把握
- 分析結果を `{{VERSIONS_ROOT}}/{{VERSION}}/requirements/existing_analysis.md` に記録

### 3. ユーザーストーリー作成

- Intent に基づいてユーザーストーリーを作成
- Epic ごとにグループ化し、優先順位を設定
- 成果物: `{{VERSIONS_ROOT}}/{{VERSION}}/story-artifacts/user_stories.md`
- テンプレート: `{{AIDLC_ROOT}}/templates/user_stories_template.md` を参照
...
- 成果物: `{{VERSIONS_ROOT}}/{{VERSION}}/story-artifacts/units/[unit_name]_definition.md`（Unit ごとに個別ファイル）
- テンプレート: `{{AIDLC_ROOT}}/templates/unit_definition_template.md` を参照

### 5. PRFAQ作成

- プレスリリース形式で製品の価値を記述
- FAQ でよくある質問に回答
- 成果物: `{{VERSIONS_ROOT}}/{{VERSION}}/requirements/prfaq.md`
- テンプレート: `{{AIDLC_ROOT}}/templates/prfaq_template.md` を参照
...
1. **計画作成**: まず実行計画を `{{VERSIONS_ROOT}}/{{VERSION}}/plans/inception_plan.md` に作成
