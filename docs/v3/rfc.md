...
- **設定キー終端値（8 か 12 か）**: 計画書内で終端値が揺れている（想定成果表・migration 表は 8、設定構成節は 12）。本 RFC は方向性として削減率 ~76%（v3 ~8）を採用するが、**v3 設定キーの確定値は data-model.md（Unit 003）の `defaults.toml` 設計で確定**する。
