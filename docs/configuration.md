...
> **後方互換性**: v2.3.6 以前から v2.4.0 にアップグレードする利用者は本キーが未設定のため、Milestone 機能は動作しません。Milestone 運用を有効化したい場合は `.aidlc/config.toml` に `[rules.milestone]` セクションと `enabled = true` を追記してください。
