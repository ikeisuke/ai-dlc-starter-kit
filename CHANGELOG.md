...
- **Inception Phase 直近サイクル完了 Unit との重複検出フロー SoT 化**: Inception Phase の新規 Unit 起案時に、直近サイクル（既定: 直前 1 サイクル）の完了 Unit 一覧と Issue 番号 / Unit タイトル / 範囲記述を機械的に突合し、重複候補があれば AI エージェントが `[Question]`/`[Answer]` 形式でユーザー確認に降ろす SoT フローを `skills/aidlc/steps/inception/04-units.md` に追加。`AskUserQuestion` で「取り下げ（`withdraw`）」「継続（`continue_with_reason`）」の二択を提示し、継続時は Unit 定義ファイル末尾に機械可読 HTML コメントブロック（`<!-- dedup-warning: source="..." related_issue="..." reason="..." detected_at="..." -->`）で重複警告を恒久記録する。v2.6.4 Unit 001 取り下げ（v2.6.3 Unit 004 と完全重複）の構造的再発防止（Unit 001 / #712）
- **Construction Phase 1 設計起草前の事前コード Read 工程組み込み**: Construction Phase 1（設計フェーズ）の設計起草前に、関連既存実装ファイル群を Read ツールで明示的に走査するフローを `skills/aidlc/steps/construction/02-design.md` に組み込み。AI エージェントが既存実装を未読のまま設計を起草し、レビュー往復で「既に実装済み」「責務分割が既存と矛盾」等の致命的指摘が頻発する問題（v2.6.4 で複数 Unit で観測）を構造的に予防する。事前 Read リストは Unit 定義の影響範囲 / 設計対象スコープから自動導出し、レビュー時に Read 完了確認を必須化（Unit 002 / #679）
- **Operations §7.13 直前マージ前完結契約最終確認プロンプト追加**: Operations Phase §7.13 PR マージ実行確認の直前に、マージ前完結契約（progress.md ステップ7「完了」更新 + 固定スロット 3 行更新 + 履歴記録 + コミット）の充足を AI エージェントが機械的に最終確認し、未充足項目があれば AskUserQuestion で「修正してから再実行 / 中断」を選択させるプロンプトを `skills/aidlc/steps/operations/operations-release.md` §7.13 に追加。マージ後の `progress.md` 編集二重更新（マージ前完結契約違反）の運用ミスを最終防衛線として構造的にブロックする（Unit 003 / #641）
- **defaults.toml 二重 SoT 同期ガード（CI 早期検出）**: `skills/aidlc/config/defaults.toml`（正本）と `skills/aidlc-setup/config/defaults.toml`（コピー）の二重 Source of Truth 同期漏れを CI で早期検出する `bin/check-defaults-sync.sh` を新設し、GitHub Actions `pr-check.yml` に組み込み。Phase 2 ゲートは dasel + jq で TOML を JSON 化し「キー集合 + 値型 + 値そのもの」の三層比較を実施（同名キーで値だけ異なる drift も `error:value-mismatch:<path>:<source_value>:<copy_value>` で検出）。`.aidlc/rules.md` の「カスタムワークフロー」セクションに「defaults.toml 同期確認」を追加し、Construction Phase で正本変更時の Unit 完了前手動チェック手順を SoT 化。プロンプトの先頭同期コメント（`# 正本:` 行）はコピー側で保持。Operations Phase でも `bin/check-defaults-sync.sh` 実行を Operations カスタムワークフローに含める運用へ統一（Unit 004 / #714）
- **/aidlc 委譲フロー Skill ツール経由自動継続実行規約化**: `/aidlc <action>`（`r` / `setup` / `migrate` / `feedback`）の独立スキル委譲経路で、親オーケストレーター（`/aidlc`）が委譲指示テキストを出力するのみで停止する従来挙動を改め、Skill ツールを経由して `/aidlc-{action}` を自動継続実行する規約に変更。`skills/aidlc/SKILL.md` の「独立フロー委譲」セクションに「Skill ツール呼び出しによる自動継続実行」を必須化し、ユーザーが委譲先スキル名を別途タイプし直す UX 摩擦を解消（Unit 005 / #717）
...
- **`aidlc-feedback` の `--web` 強制起動解消（opt-in 化）**: `gh issue create` 実行時に `--web` フラグが常時付与され、ブラウザが必ず開いてしまう挙動を opt-in 化。デフォルトは CLI 完結（環境変数 `AIDLC_FEEDBACK_WEB=1` または `--web` フラグ明示時のみブラウザ起動）（Unit 003 / #690）
...
- 必須 Checks の常時 PASS 報告化を `.github/workflows/pr-check.yml` / `migration-tests.yml` / `skill-reference-check.yml` に追加。`paths:` フィルタおよび job-level の Draft `if:` を削除して既存ジョブを常時起動させ、Detect skip step による step-level スキップ制御に置換することで、対象外パス・Draft 状態でも各 workflow の job 自体は success を報告する。これによりブランチ保護で PR が merge 不可になる事象を解消（#598 / Unit 002）
...
- Milestone 関連 4 ファイル（`skills/aidlc/steps/inception/02-preparation.md` §16 / `inception/05-completion.md` §1 / `operations/01-setup.md` §11 / `operations/04-completion.md` §5.5）の責務境界・参照関係を明確化。`SELECTED_ISSUES` 保持の明示、`MILESTONE_ENABLED` × 非空 `SELECTED_ISSUES` の AND ガード明示、`MILESTONE_NUMBER` 抽出例追加、`setup-step11` 内部処理注記、相互参照整合確認を実施（empirical-prompt-tuning 由来の構造審査指摘 5 件、中優先度 1 件 / 低〜中 1 件 / 低 2 件 + 軽微確認 1 件を最小修正案で解消）（#602 / Unit 005）
...
- Operations Phase開始時にリモート同期チェックを追加。`git fetch` + `git rev-list` でリモートとの差分を検出し、behind件数が1以上の場合はユーザーに取り込み/スキップを確認（#571）
- SKILL.mdの「実行判断・対話規約」セクションに推奨・提案応答確保ルールを追加。応答を待つか進行して記録するかの使い分け基準を定義（#570）

### Fixed

- プリフライトチェックの `config/defaults.toml` 存在確認がプロジェクトルート相対パスで実行され偽陰性の警告が発生する問題を修正。スキルベースディレクトリからの相対パスで解決するよう変更（#569）

### Changed

- `steps/inception/01-setup.md` のmain取り込み推奨にユーザー応答フローを追加（#570）
- `steps/common/preflight.md` の警告表示後に続行判断フローを追加（#570）
...
- 未使用設定キー `cycles_dir` を `config.toml`・`defaults.toml`・セットアップテンプレートから削除（#506）
- `rules.cycle.named_enabled` 設定キーを追加し、名前付きサイクル機能をデフォルト無効（opt-in）に変更（#507）
- AskUserQuestion ツール使用ルールを `steps/common/rules.md` に追加し、ゲート承認とユーザー選択の区別を明記（#505）
- `/aidlc version`（短縮形: `v`）アクションを追加し、スターターキットバージョンの確認が可能に（#508）
- フェーズ・Unit完了時にコンテキストリセット提示の前に完了サマリ（成果物一覧・決定事項・レビュー結果・変更ファイル等）を表示するよう改善
...
- バージョン比較ロジックを三角モデル（リモート/インストール済みスキル/ローカル設定）に拡張し、全7パターンの比較・アクション提示に対応（#503）
- Unit完了時・フェーズ完了時のコンテキストリセット提示に対応内容サマリ（実施内容・変更対象・未対応事項・次回着手点）を追加（#502）
- 全フェーズでセットアップ完了直後にタスクリストを作成するよう指示位置を前倒し
- Inception Phaseでprogress.mdをサイクルディレクトリ作成直後に作成するよう前倒し

### Changed

- `.aidlc/rules.md` の「aidlc-setup同期」カスタムワークフローを廃止（三角モデルに統合）
- アップグレード案内時のstarter_kit_version確認手順を明示化
...
- バックログ登録時のスコープガード追加: intent.mdのスコープ定義と照合し、スコープ内作業がバックログに外出しされることを防止（#401）
- semi_autoゲートにレビュー実施済み前提条件を追加: フォールバック条件テーブルに`review_not_executed`（優先度0）を追加し、AIレビュー未実施での自動承認を防止（#400）
- `reason_code`有効値をバリデーション規則に明記

### Fixed

- aidlc-setup.shのmigrate-config警告検出をstdout解析に移行: 終了コード規約変更後も警告を正しく検出できるように修正（#402）
- Codex PRレビュー初回自動トリガーのルールを明記
...
- エクスプレスモード再設計: depth_levelとの結合を解除し、複雑度判定による自動制御を導入。Unit数1の場合に自動適用される仕組みに変更（#396相当）
- 設計レビュー時のガイド照合ルールを追加: 既存ガイド文書との整合性確認を設計レビューの観点に含めるように `rules.md` を更新

### Fixed

- シェルスクリプト終了コード規約統一: 警告付き完了はexit 0を維持するように修正し、exit 1/2の混在問題を解消（#397）
- Codex PRレビュー設定追加: `rules.md` にCodex連携のレビュールールを追記

### Added

- 外部依存ドキュメント: README.mdに外部リポジトリ（claude-skills）の依存関係を記載（#396）
...
- Kiroエージェント許可設定の自動セットアップ: `setup-ai-tools.sh`の`setup_kiro_agent`関数を拡張し、許可設定のテンプレート管理とマージ機能を実装（#385）
- ワイルドカードルール検出による重複防止: パーミッションマージ時に既存ワイルドカードルールに包含される個別ルールの追加をスキップ（#384）
...
- フリクション対処トラブルシューティングガイド: AI-DLC利用時によくある5カテゴリのフリクションパターンと対処法をまとめたガイドを新規作成（#324）

### Changed

- `post-merge-cleanup.sh` 通常ブランチ対応: worktree環境だけでなく通常ブランチ（非worktree環境）でもマージ後のクリーンアップ処理を実行可能に（#377）
...
- トークン数ベースサイズチェック: `bin/check-size.sh` にトークン数ベースの評価を追加。ハイブリッド方式（近似計算 + tiktoken）で、tiktoken未インストール時も近似計算で動作（#349）
- `--tokens-threshold` CLIオプション: トークン数閾値をコマンドラインから指定可能に
- `max_tokens` 設定項目: `aidlc.toml` の `[rules.size_check]` セクションにトークン数閾値設定を追加（デフォルト: 40000）
...
- エクスプレスモード インスタント起動: 「start express」コマンドでaidlc.toml変更なしにエクスプレスモードを直接起動可能に（#364）
- プリフライトチェック・設定値一括提示: 各フェーズ開始時にツール存在・認証状態を自動チェックし、主要設定値を一括提示する共通プリフライト機構を実装（#320）
- Codex PRレビュー絵文字リアクション検出: PRマージ前ゲートにCodex PRレビューの絵文字リアクション（👍/👀）検出を追加（#336）

### Changed

- .kiro/skills → .agents/skills 移行: スキルディレクトリを `.kiro/skills` から `.agents/skills` に移行し、全参照を更新
...
- rules.md: ローカルレビュー必須ルールを追加（外部レビューツール呼び出し前にgit diffで差分確認を義務化）
...
- 名前付きサイクル履歴修正: `write-history.sh` が名前付きサイクル（`name/vX.X.X` 形式）のパスを正しく解決できるように修正（#301）

### Added

- main最新化チェック: `setup-branch.sh` に `origin/main` との差分検出機能を追加し、ブランチ作成時の最新化状態を出力（#307）
- プロンプトブランチ管理改善: Inception Phase・Operations Phaseにmain最新化チェック表示、非正規ブランチ時のサイクルブランチ提案、名前付きサイクル候補表示を追加（#302, #303）
- バックログ名前付きサイクル情報付与: バックログ登録時に名前付きサイクルの「発見サイクル」情報が正しく記録されるようプロンプトを明確化（#305）
- ツールチェック回避設定: `tools = []`（空配列）設定時に外部CLIのwhichチェックをスキップし、セルフレビューモードに直接遷移するロジックを追加（#306）
...
- セキュリティレビュー観点拡充: reviewing-securityスキルにAmazon AIDLC SECURITY-01〜15の全15ルール対応観点を追加。アクセスログ、ネットワーク構成、セキュアデザイン原則、ソフトウェア整合性検証、アラート/監視、例外処理の各カテゴリを網羅（#278）
- コンテンツバリデーション: reviewing-codeスキルにASCII図・Mermaid図のバリデーション観点を追加。図表の構文検証・整合性チェック・アクセシビリティ考慮を含む（#279）
- テンポラリファイル出力先規約: `common/rules.md` にテンポラリファイルの出力先ディレクトリ統一規約を追加

### Changed

- 監査ログ強化: `write-history.sh` の出力にISO 8601タイムスタンプを付加し、構造化ログを強化。既存のMarkdownフォーマットとの後方互換性を維持（#277）
- テンポラリファイルパス統一: CLAUDE.md、commit-flow.md、review-flow.md、squash-unit/SKILL.md等の固定パスを出力先規約に統一
