...
- **「1. はい」**: 全キーを `config.toml` に追記する。dasel v2/v3でコマンド形式が異なるため、以下の順で試行する:
  - v2: `dasel put -f .aidlc/config.toml -t <type> '<key>' -v '<value>'`（値にシングルクォート等の特殊文字が含まれる場合は適切にエスケープすること）
  - v3またはdasel失敗時: v3では `put` サブコマンドが廃止されているため、AIが直接TOMLファイルを編集して追記する（Editツール等を使用）
  - 追記後に `追記完了: N 件のキーを追加しました` と表示する
