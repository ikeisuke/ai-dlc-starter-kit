...
1. プロジェクトルート（`.aidlc/config.toml` が存在するディレクトリ）で `dasel -f .aidlc/config.toml 'starter_kit_version'` を実行し、ローカルバージョンを取得する。取得値は引用符除去・前後空白トリムで正規化し、正規化後に空文字ならフェイルセーフ（失敗扱い）とする
