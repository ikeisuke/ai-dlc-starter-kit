...
> **位置づけ（v3.0.0-alpha.7 / Phase 6）**: 本ファイルは status コマンドの**出力仕様 + 実行手順**である。
...
>
> **コマンド表記**: v3 の現在の起動表面は `/aidlc-v3`（v2 共存 / `SKILL.md`「コマンド表記について」）。
> 出力例・Suggested command は `/aidlc-v3` 表記を用いる（最終表面 `/aidlc` への統一は Phase 7）。
...
2. **schema 検証**（`state-read.sh` は schema 妥当性を検証しないため `state-validate.sh` に委譲）:
   ...
   - `status:warn:unsupported-schema-version:*`（未対応 schema / `data-model.md` §6 の復帰不可 WARN）/
     ...
     ```text
     state read error
     Suggested command: /aidlc-v3 doctor
     ```
...
3. **cycle 解決 + 安全検証**:
   ...
   - 空 / 取得失敗 / 不正識別子 / cycle ディレクトリ不在 → **No active cycle にせず**、上記の `state read error`
     診断案内（`Suggested command: /aidlc-v3 doctor`）を出力して終了。
...
| `Suggested command` | 導出フェーズに対応する次コマンド（`/aidlc-v3 develop` / `/aidlc-v3 release` / `/aidlc-v3 reflect` 等） |
