...
1. **current_cycle 解決**: `<cycle>` プレースホルダを推測・手動置換せず、`state-read.sh` で解決する:
   ...
   - exit 1（`state.json` 不在 / `current_cycle` 欠落 = active cycle なし）→ 「先に `/aidlc-v3 define` を実行してください」と案内して**終了**。
...
2. **`complete` 前提確認**（reflect は complete 状態でのみ実行可 / `docs/v3/data-model.md` §5.1）:
   ...
   - `release.merge_approved` が `true` かつ `release.pr_number` が整数 → PR merged 実態を確認する:
     ...
     - `MERGED` でない / 取得不能 → 「このサイクルはまだ完了（merge 済み）していません。先に `/aidlc-v3 release` を完了してください」と案内して**終了**。
   - `release.merge_approved` が `true` でない / `release.pr_number` 欠落 → 「reflect は release 完了後に実行します。`/aidlc-v3 status` で現在地を確認してください」と案内して**終了**（mutation なし）。
...
`docs/v3/data-model.md` §10・`docs/v3/workflow.md` §3.4）。`/aidlc-v3 status` で現在地を確認できる。
