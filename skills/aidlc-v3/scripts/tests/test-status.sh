...
#   (3) §3.5 ブロックとのフィールド構造一致 / (4) launch prefix /aidlc-v3 統一 /
...
if grep_q "$STATUS_MD" "Suggested command: /aidlc-v3 define"; then pass "define 案内 exact string"; else fail "define 案内 exact string 欠落"; fi

echo "== §3.5 出力例ブロックのフィールド列 exact 比較（workflow.md ↔ status.md） =="
# active 出力例ブロック（Cycle: 〜 Suggested command:）から「ラベル:」列を順序付きで抽出する。
# フィールドラベルは prefix（/aidlc vs /aidlc-v3）非依存のため、ラベル列の exact 一致で
...
echo "== launch prefix /aidlc-v3 統一（Suggested command） =="
# status.md の Suggested command 行はすべて /aidlc-v3 を使う（bare /aidlc を Suggested command に使わない）。
sugg_bad="$(grep -F "Suggested command:" "$STATUS_MD" | grep -cE "/aidlc[^-]" || true)"
if [[ "$sugg_bad" -eq 0 ]]; then pass "Suggested command はすべて /aidlc-v3（bare /aidlc 混在なし）"; else fail "Suggested command に bare /aidlc が混在（${sugg_bad} 件）"; fi
...
if grep_q "$STATUS_MD" "Suggested command: /aidlc-v3 doctor"; then pass "読取失敗時 doctor 案内"; else fail "doctor 案内欠落"; fi
