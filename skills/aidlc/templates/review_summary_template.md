...
> **注（v2.5.4 以降）**: 以下の例の `内容` 列に「最大3回の反復制限を追記」など過去サイクルの履歴文言が含まれている場合がある。**反復回数上限**: v2.5.2 で 5 回に拡張（v2.5.4 でも維持）。**完了条件の sequential history**: v2.5.2 で `(1R clean 特例) OR (最後 2 round 連続で指摘ゼロまたは defer 化)` を導入 → v2.5.4 Unit 005 で `(1R clean 特例) OR (rounds.size >= 2 && last_round_clean)` に緩和（Round 1 指摘 → Round 2 全 resolve の 2R 完了を許可）。詳細は `review-flow.md` 参照。例文中の「3 回」表記は当時の上限値の記録であり、現行仕様ではない。
...
> 上記 #2 の「最大3回の反復制限」は当時の上限値の記録。v2.5.2 以降は 5 回上限、v2.5.4 以降は完了条件が `last_round_clean` ベース（直近 round が clean なら完了）に緩和（`review-flow.md` 参照）。
