...
```bash
scripts/resolve-starter-kit-path.sh
```
...
<!-- AIDLC-DEPRECATED: sync-package.sh はv2で廃止。プラグインモデルでは不要 -->
**同期スクリプト**:
...
| # | ソース | 宛先 | 内容 |
|---|--------|------|------|
| 1 | `[スターターキットパス]/prompts/package/templates/` | `templates/` | ドキュメントテンプレート |
| 3 | `[スターターキットパス]/prompts/package/guides/` | `docs/aidlc/guides/` | ガイド |
| 4 | `[スターターキットパス]/prompts/package/bin/` | `scripts/` | スクリプト |
