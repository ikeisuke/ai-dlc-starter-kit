...
2. **current_cycle 解決**: 永続化された active cycle は `state.json` のみが保持する。`<cycle>` プレースホルダを
   ...
   - exit 1（`state.json` 不在 / `current_cycle` 欠落 = active cycle なし）→ 「先に `/aidlc define` を実行してください」と案内して**終了**（mutation なし）。
...
- exit 0 + `true` → 1-2 へ進む。
- exit 0 + `false` → 「define が未完了です。`/aidlc develop` で work item を完了してから release してください」と案内して**終了**。
...
- exit 1（`state.json` 不在 / キー欠落）→ 「active cycle がありません。先に `/aidlc define` を実行してください」と案内して**終了**（Step 0 で解決済みでも防御的に扱う）。
...
導出される（`docs/v3/data-model.md` §5.1 評価順 1 / 本ファイルは導出規則を再定義せず参照）。`/aidlc status` で確認できる。
