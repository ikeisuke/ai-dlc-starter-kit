...
設計起草前に改修対象機能の既存実装コードを Read し、ドメインモデル / 論理設計の冒頭に以下 3 観点を記述する。これは「既存実装の挙動を読まずに設計起草」を主因とする Round 1 設計レビュー反復を構造的に予防するための工程（v2.6.5 / #679 / Unit 002 で導入）。

### (a) Read 対象ファイル + 目的

改修予定の既存実装ファイル（パスと Read 目的の 1 文）を表形式で列挙する。consumer プロジェクト由来のファイルも含めて、設計判断の根拠となる範囲をすべて挙げる。

### (b) 設計時に意識すべき挙動

既存実装のエッジケース・副作用・依存関係・後方互換上の制約など、設計起草時に踏まえるべき挙動を箇条書きで記述する。

### (c) 既存実装に基づく代替案検討

`refactor` / `extend` / `replace` 等の主要な実装方針候補について、既存実装との適合性（採用 / 却下 + 根拠）を比較記述する。

**失敗時アクション**: 設計レビュー (`reviewing-construction-design`) の「### 設計プロセス」観点で「事前コード読込みセクション存在 / 内容充足」が不足判定された場合、修正されるまで Round 反復する。
