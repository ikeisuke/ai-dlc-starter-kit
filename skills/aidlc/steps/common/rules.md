...
# 例
scripts/read-config.sh rules.reviewing.mode
scripts/read-config.sh --keys rules.reviewing.mode rules.squash.enabled
```

**モードの使い分け**:
- 単一キーモード: 1つの設定値を取得。`config/defaults.toml` のデフォルト値が自動的にフォールバックされる
...
**レッドフラグ**（行動を見直す）: 複雑なプロジェクトで質問なしにステージ完了、曖昧な回答で作業進行、「質問すべき場面」のスキップ、質問の代わりに仮定を使用

**成功指標**: 複雑さに見合った質問量、回答の徹底分析と追加質問、実装前の要件明確化
...
```bash
scripts/read-config.sh rules.upgrade_check.enabled
```
...
```bash
scripts/read-config.sh rules.depth_level.level
```
...
|---------|--------|---------|----------|---------------|
| Inception | Intent | 1-2文の簡潔な記述 | 標準的な記述（背景・目的・スコープ） | 詳細な記述 + リスク分析・代替案検討 |
| Inception | ユーザーストーリー | 主要ケースのみ簡略化 | 完全な受け入れ基準（INVEST準拠） | 完全な受け入れ基準 + エッジケース網羅 |
| Inception | Unit定義 | 最小限の責務・境界 | 完全な責務・境界・依存関係 | 完全な記述 + 技術的リスク評価 |
| Inception | PRFAQ | スキップ可能 | 通常通り | 通常通り |
| Construction | ドメインモデル | スキップ可能（設計省略を明記） | 標準的なドメインモデル設計 | 詳細なドメインモデル + ドメインイベント定義 |
| Construction | 論理設計 | スキップ可能（設計省略を明記） | 標準的な論理設計 | 詳細な論理設計 + シーケンス図・状態遷移図 |
| Construction | コード・テスト | 通常通り | 通常通り | 通常通り + 統合テスト強化 |
| Operations | リリース準備 | 通常通り | 通常通り | 通常通り + ロールバック手順の詳細化 |
...
**「通常通り」の定義**: テーブル内の「通常通り」は、該当成果物に対して特別な簡略化・追加要件がないことを意味する。各フェーズの標準的なワークフロー（ステップファイルの手順）がそのまま適用される。
...
```bash
scripts/read-config.sh rules.construction.max_retry
```
...
```bash
scripts/read-config.sh rules.preflight.enabled
```
...
```bash
scripts/read-config.sh rules.preflight.checks
```
...
- `mktemp`により一意パス生成（シンボリックリンク攻撃・ファイル衝突防止）
- 一時ファイルに**機密情報を含めない**（用途は非機密情報に限定）
- 規約外パスは明示的な技術的要件がある場合のみ許可（理由をコメントで明記）

### セミオート履歴記録フォーマット

`mktemp`でパス生成→Readツール→Writeツールで履歴コンテンツ書き込み→`scripts/write-history.sh`実行→一時ファイル削除。`--unit`/`--unit-name`/`--unit-slug`はconstructionフェーズのみ指定。

**自動承認時**（ステップ名: `セミオート自動承認`）:
...
**禁止**: 改善提案テキストのみでissue/ファイル未作成。**正しい**: 提案と同時にバックログIssue作成（`#XXX`を提示）。
