...
| gh_status | 対象機能の動作 |
|-----------|--------------|
| `available` | Issue クローズ・PR 操作・タグ push・GitHub Release 作成を実行。Milestone 紐付け確認 / Milestone close は **`[rules.milestone].enabled=true` のときのみ実行**（既定 off では `01-setup.md` ステップ 11 / `04-completion.md` ステップ 5.5 自体がスキップされる、Unit 008 / #597 Unit G） |
