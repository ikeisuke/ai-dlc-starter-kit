...
14. 7.13 PRマージ【ユーザー選択: automation_mode に関わらずユーザー確認必須】
    - サブステップ順序: マージ方法確定 → 設定保存フロー → 未コミット差分検出ガード → **マージ前完結契約最終確認**（v2.6.5 / #641 / Unit 003 で追加 / `automation_mode` 非依存・例外なし）→ マージ実行確認 → `gh pr merge` 実行
    - 文言・選択肢・例外条件の SoT は `operations-release.md §7.13` を参照（本ファイルは目次責務）
