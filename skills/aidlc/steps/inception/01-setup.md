...
**判定**:
- **STARTER_KIT_DEV**: 以下を表示し、ステップ5（追加ルール確認）へ進む（ステップ6はスキップ）
  ```text
...
**スキップ条件**: ステップ4で `STARTER_KIT_DEV` と判定された場合、このステップをスキップしてステップ7（サイクルモード確認）へ進む。

**アップグレードチェック設定の確認**:

`common/rules.md` の「アップグレードチェック設定」セクションに従い、設定値を取得する。

`false` の場合（デフォルト）: 以下を表示し、ステップ7（サイクルモード確認）へ進む:
...
**判定**:
- **最新バージョン取得失敗**: ステップ7（サイクルモード確認）へ進む
- **CURRENT_VERSION が空**: ステップ7（サイクルモード確認）へ進む（aidlc.tomlなし）
- **LATEST_VERSION > CURRENT_VERSION**: アップグレード推奨を表示
  ...
  - **1 を選択**: セットアップを案内して終了
    ...
    ```
  - **2 を選択**: ステップ7（サイクルモード確認）へ進む
- **LATEST_VERSION = CURRENT_VERSION**: 以下を表示し、ステップ7（サイクルモード確認）へ進む
  ```text
...
- 既存名を選択した場合: 選択された名前を `cycle_name` としてコンテキスト変数に保持し、ステップ9へ進む
...
**バリデーション成功時**: `cycle_name` をコンテキスト変数として保持し、ステップ9へ進む。
...
**スキップ条件**（以下のいずれかに該当する場合、このステップをスキップしてステップ9へ進む）:

- `cycle_name` が既に設定済みの場合（ステップ7で名前入力済みまたは既存名を選択済み）
- ステップ7で `mode = "ask"` から「1. 通常サイクル」を選択した場合
...
- `cycle_mode` を `"named"` に更新する（ステップ9以降の分岐との整合性を維持するため）
- ステップ9（サイクルバージョンの決定）へ進む（名前付きサイクルモードのバージョン提案フローが適用される）

**「1. いいえ」選択時**:

- ステップ9（サイクルバージョンの決定）へ進む（通常フロー）

#### 9. サイクルバージョンの決定

**9-1. コンテキスト表示**（バージョン提案前に実行）:
...
4. マッチしたバージョンがない場合（名前付きサイクルの初回）: スクリプトの `suggested_*` はグローバルサイクル基準のため使用しない。固定値 `v0.0.1` / `v0.1.0` / `v1.0.0` の3択を提示
...
- **main または master の場合**: ブランチ作成方式の設定を確認し、方式に応じて処理
  ...
  **10-4. main最新化チェック結果の表示**（setup-branch.sh実行する全経路 — branch/worktree/ask — に共通で適用）:
...
- **それ以外のブランチまたはdetached HEAD**:
  ...
  **既存のサイクルブランチがある場合**:
  ...
  - 「1. 新しいサイクルブランチを作成する」選択時: 上記の main/master ブランチの場合と同じブランチ作成フロー（10-1〜10-4）を実行
...
**10-5. rules.md再読み込み【ブランチ切り替え時】**（全ブランチ切り替え経路に共通で適用）:

ステップ10でブランチ切り替えが発生した場合（setup-branch.sh実行 または git checkout実行の経路を通った場合）、ステップ5（追加ルール確認）を再実行する。
...
- **存在しない場合**: ステップ12（サイクルディレクトリ作成）へ進む
