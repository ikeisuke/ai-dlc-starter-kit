...
| main_status | メッセージ | 応答 |
|-------------|-----------|------|
| `up-to-date` | 最新です | 不要（情報提供のみ） |
| `behind` | ⚠ リモートデフォルトブランチに未取り込みコミットがあります。 | `AskUserQuestion` で「取り込む / スキップして続行」を選択（SKILL.md「推奨・提案応答確保ルール」参照）。「取り込む」選択時: ユーザーに手動で `git merge` または `git rebase` を依頼し、完了後にステップ9-3を再実行して `up-to-date` を確認する |
| `fetch-failed` | リモート確認失敗（オフライン等） | 不要（スキップして続行） |
