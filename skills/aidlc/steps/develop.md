...
レビュー完了後、`skills/aidlc/steps/common/review-flow.md`「レビュー完了時の共通処理」のセミオートゲート判定
（`unresolved_count == 0` かつフォールバック非該当 → `auto_approved`）に従い Step 6 へ進む。残指摘ありは review-flow.md の指摘対応判断フローで処理する。
...
- `define_completed: true` かつ done / withdrawn 以外の work item が残る（評価順 3）→ **develop 継続**。
  次の着手候補があれば `/aidlc develop` を案内する。
