...
| レベル | 実装例 | 特徴 |
|--------|-------|------|
| アプリケーションレベル | Claude Codeのパーミッション | ツール内蔵の権限制御、設定が簡単 |
...
| 通常開発作業 | アプリレベル中心 | `-v $(pwd):/workspace:rw` + Claude Codeパーミッション |
...
| ユースケース | 推奨設定 | ツール別設定 |
|-------------|---------|-------------|
| コードレビュー・調査 | 読み取り専用 | Claude Code: Docker `:ro`マウント |
| 通常の開発作業 | ワークスペース書き込み | Claude Code: 特定ディレクトリのみマウント |
