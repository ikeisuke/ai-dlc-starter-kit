...
#     <fixture>/skills/aidlc/scripts/    … 実 v3 スクリプト + doctor.sh をコピー
...
SCRIPTS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"             # skills/aidlc/scripts
...
#   build_fixture <root>
# 生成物:
#   <root>/skills/aidlc/scripts/{doctor.sh + 実 v3 スクリプト群}
...
build_fixture() {
    local root="$1"
    local v3="$root/skills/aidlc/scripts"
...
rm -f "$ROOT/skills/aidlc/scripts/state-init.sh"
run_doctor "$DOCTOR" "$ROOT" PATH="$ROOT/bin:$PATH"
...
cat > "$ROOT/skills/aidlc/scripts/work-item-validate.sh" <<'STUB'
#!/usr/bin/env bash
exit 2
STUB
chmod +x "$ROOT/skills/aidlc/scripts/work-item-validate.sh"
run_doctor "$DOCTOR" "$ROOT" PATH="$ROOT/bin:$PATH"
