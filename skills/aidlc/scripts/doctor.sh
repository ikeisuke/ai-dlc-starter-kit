...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"        # skills/aidlc/scripts
V3_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"                            # skills/aidlc
...
diagnose_state() {
    if [[ ! -f "$STATE_FILE" ]]; then
        report state WARN "No active cycle（/aidlc define で開始）"
        STATE_PRESENT=0
        return
    fi
    STATE_PRESENT=1
...
diagnose_work_items() {
    ...
    if [[ ! -d "$wi_dir" ]]; then
        report work-items WARN "work-items ディレクトリが未作成（/aidlc define 前）"
        return
    fi
