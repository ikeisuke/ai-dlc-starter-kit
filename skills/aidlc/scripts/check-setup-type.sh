...
# 設定ファイルの存在確認（v2 → v1 → 旧形式の順で判定）
# v1形式が存在する場合もv2変数に統一して処理
if [ ! -f "$AIDLC_TOML" ] && [ -f "$AIDLC_TOML_V1" ]; then
    AIDLC_TOML="$AIDLC_TOML_V1"
fi

if [ -f "$AIDLC_TOML" ]; then
    # 設定ファイルが存在 - check-version.sh を呼び出してバージョン状態を取得
